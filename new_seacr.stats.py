...
def combine(list_1, list_2):
    ...
    Returns:
        Sorted list
    """
    return np.array(sorted(set(list_1).union(set(list_2))))
...
def pct_remain_vec(vec_1, vec_2, x):
    ...
    ecdf_1 = ECDF(vec_1)
    ecdf_both = ECDF(both)
    top = len(vec_1) - ecdf_1(x) * len(vec_1)
    bottom = len(both) - ecdf_both(x) * len(both)
    #top = top[~np.isnan(top)]
    #bottom = bottom[~np.isnan(bottom)]
...
def find_relaxed_thresh(exp, ctrl):
    ...
    outer = pct_remain_vec(exp, ctrl, both[inner < 1])[:-1]
