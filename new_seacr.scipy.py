...
def find_relaxed_thresh(exp, ctrl):
    ...
    outer = pct_remain_vec(exp, ctrl, [both[_] for _ in indices])
    raise Exception(both[outer.argmax()])
...
#' Summarize AUC data.
#'
#' @param vec A vector of AUC
#' @return A data.frame with columns count, quant, value, diff, and dist.
get_dynamic_frame <- function(vec) {
	vec <- sort(vec, decreasing=TRUE)
	vec.max <- max(vec)
	vec.frame <- data.frame(
		count=seq(1, 0, length=length(vec)),
		quant=vec / vec.max,
		value=vec
	)
	vec.frame$diff <- abs(vec.frame$count - vec.frame$quant)
	vec.frame <- vec.frame[vec.frame$diff > 0.9 * max(vec.frame$diff),]
	vec.frame$dist<-apply(vec.frame, 1, dist2d)
	return(vec.frame)
}
...
#'
#' @param vec A vector of AUC
#' @return A data.frame with columns values and quantile
get_static_frame = function(vec) {
	quantile <- 1 - ecdf(vec)(vec)
	data.frame(values=vec, quantile=quantile)
}


#' Find the ratio of two maxs' distributions.
#'
#' @param vec.1 A vector
#' @param vec.2 A vector
#' @return The difference between the ecdf for vec.2 and vec.1 at x
pctremain.max <- function(vec.1, vec.2, x) {
	1 + ecdf(vec.2)(x) - ecdf(vec.1)(x)
}

#' Find the value associated with the farthest distance.
#'
#' @param vec A vector
#' @param vec.frame A data.frame produced from get_dynamic_frame(vec)
#' @return A numeric value
get_farthest_value <- function(vec) {
	frame <- get_dynamic_frame(vec)
	max(
		frame[which.max(frame$dist), "value"],
		quantile(vec, probs=0.9, names=FALSE)
	)
}
...
#'
#' @param vec A vector
#' @return A numeric value between 0.99 and 1
find_best_quantile = function(vec) {
	i <- 2
	while(quantile(vec, 1 - 1 / 10 ** i, na.rm=TRUE) == 0) i <- i + 1
	return(1 - 1 / 10 ** i)
}

#' Find outlier AUC values.
#'
#' @param exp.vec Experimental vector
#' @param ctrl.vec Control vector
#' @return A subset of exp.vec and ctrl.vec
spurious_values = function(exp.vec, ctrl.vec) {
	both <- sort(unique(c(exp.vec, ctrl.vec)))
	diff <- abs(diff(pctremain.vec(
		exp.vec,
		ctrl.vec,
		both)))
	thresh <- both[-length(both)]
	q <- find_best_quantile(diff)
	thresh[diff != 0 & diff < quantile(diff, q, na.rm=TRUE)]
}
...
#' @param exp.vec Experimental vector
#' @param ctrl.vec Control vector
#' @return New relaxed and stringent thresholds if needed, else FALSE
thresh_check = function(exp.vec, ctrl.vec) {
	sp.values <- spurious_values(exp.vec, ctrl.vec)
	sp.pct <- pctremain.vec(exp.vec, ctrl.vec, sp.values)
	search.values <- pctremain.vec(exp.vec, ctrl.vec, sp.values[sp.pct < 1])
	thresh.sp <- sp.values[which.max(search.values)]
        thresh.check <- find_stringent_thresh(exp.vec, ctrl.vec, thresh.sp)
	both <- sort(unique(c(exp.vec, ctrl.vec))) ## New for SEACR_1.1
	both.pct <- pctremain.vec(exp.vec, ctrl.vec, both)
	old.values <- pctremain.vec(exp.vec, ctrl.vec, both[both.pct < 1])
	if(max(search.values, na.rm=TRUE) / max(old.values, na.rm=TRUE) > 0.95) {
		return(c(thresh.sp, thresh.check))
	}
	return(FALSE)
}
...
#' Find the global threshold.
#'
#' @param exp.max Experimental vector
#' @param ctrl.max Control vector
#' @return A numeric value
find_global_thresh = function(exp.max, ctrl.max) {
	both2<-c(exp.max,ctrl.max)
	d <- sort(unique(both2))
	d.pct <- pctremain.max(exp.max, ctrl.max, d)
	if(any(d.pct > 1)) {
		d0 <- min(d[d.pct > 1])
	} else {
		d0 <- 1
...
#' Find the threshold from a single vector.
#'
#' @param frame A data.frame from get_static_frame(vec)
#' @param ctrl The maximum quantile cutoff
#' @return A numeric value
get_static_thresh = function(frame, ctrl) {
	min(frame[frame$quantile < ctrl, "values"])
}

#' Find the thresholds given only an experimental vector
#'
#' @param exp.vec Experimental vector
#' @param exp.max Experimental max vector
#' @param ctrl A numeric values between 0 and 1
#' @return Relaxed threshold, stringent threshold, global threshold, and fdr
get_static_thresholds = function(exp.vec, exp.max, ctrl) {
	frame.vec <- get_static_frame(exp.vec)
	frame.max <- get_static_frame(exp.max)
	ctrl <- as.numeric(ctrl)
	thresh.r <- get_static_thresh(frame.vec, ctrl)
	thresh.s <- get_static_thresh(frame.max, ctrl)
	thresh.g <- 0
	fdr <- ctrl ## New for SEACR_1.1

	return(c(thresh.r, thresh.s, thresh.g, fdr))
}
"""
